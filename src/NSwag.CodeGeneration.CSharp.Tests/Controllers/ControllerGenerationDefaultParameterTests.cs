...
namespace NSwag.CodeGeneration.CSharp.Tests.Controllers
{
    public class ControllerGenerationDefaultParameterTests
    {
        ...
        public void When_parameter_has_default_then_set_in_partial_controller()
        {
            ...
            // Assert
            Assert.Contains("_implementation.GetsFooBarAsync(abc, booldef ?? true, intdef ?? 42, doubledef ?? 0.6822871999174D, decdef ?? 79228162514264337593543950335M, strdef ?? \"default\\\"string\\\"\", bar)", code);
            Assert.Contains("GetsFooBarAsync(int abc, bool booldef, int intdef, double doubledef, decimal decdef, string strdef, int? bar = null);", code);
